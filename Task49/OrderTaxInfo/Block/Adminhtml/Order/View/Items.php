...
class Items extends BaseItems
{
    ...
    public function getNepalGSTTaxAmount()
{
    $order = $this->getOrder();
    $nepalGstTax = 0.0;

    // Loop through the order's tax items to find the Nepal-GST tax
    foreach ($order->getExtensionAttributes()->getAppliedTaxes() as $tax) {
        if ($tax->getCode() == 'Nepal-GST') {
            $nepalGstTax = $tax->getAmount();
            break;
        }
    }
    ...
    public function hasGstOrPst()
    {
        return $this->getNepalGSTTaxAmount() > 0 || $this->getCanadaPSTTaxAmount() > 0;
    }
}
